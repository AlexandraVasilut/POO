...
bool Game::regame()
{
    ...
    std::cout << "Vrei sa te joci iar? Tasteaza 'y' daca da sau 'n' pentru nu (y/n) " << std::endl;
    char c{};
    bool isvalid{false};
    do
    {
        std::cin >> c;
        if (c == 'y')//daca scrii 'y' reincepe jocul
            return true;
        else
            if (c == 'n')//daca scrii 'n' se inchide jocul
                return false;
        else
        {
            std::cout << "Raspuns invalid\n";//daca scrii alta tasta fata de y sau n apare acest text
            isvalid = true;
        }
    } while(isvalid);
}
...
void Game::controls()
{
    if (GetAsyncKeyState(VK_UP))//cu sageata sus se roteste forma
            checked_rotate();
        else
            if (GetAsyncKeyState(VK_DOWN))//cu sageata jos coboara piesa mai jos
                speed = 10;
        else
            if (GetAsyncKeyState(VK_RIGHT))//cu sageata dreapta muti piesa in partea dreapta
                checked_move(right);
        else
            if (GetAsyncKeyState(VK_LEFT))//cu sageata stanga muti piesa in partea stanga
                checked_move(left);
}

void Game::ClearScreen()//functia care curata ecranul pentru un nou joc
{
